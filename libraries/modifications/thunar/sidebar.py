...
# The "Home Directory" bookmark (PROMPT batch item 4). Thunar's built-in Home shortcut shows
# the bare username ("main") -- the home GFile's display-name, which has no rename pref. So the
# built-in Home is HIDDEN (settings.HIDDEN_BOOKMARKS carries file:///home/main) and we ship THIS
# labelled bookmark instead. CRUCIAL: it points at the HIDDEN `.home-directory -> .` symlink
# (home_directory.HOME_DIR_BOOKMARK_URI), NOT straight at file:///home/main -- because Thunar
# hides bookmarks by CANONICAL URI, so a bookmark at file:///home/main would be hidden by the
# same hidden-bookmarks entry that removes the built-in Home (the two URIs are identical). The
# .home-directory symlink gives a DISTINCT URI that survives the hiding yet resolves INTO the
# home dir when clicked. The trailing text after the URI is the GTK bookmark label Thunar
# renders. Placed FIRST so Home is at the top of the shortcuts pane.
HOME_BOOKMARK_LABEL = "Home Directory"
HOME_BOOKMARK_URI = home_directory.HOME_DIR_BOOKMARK_URI
...
def gtk_bookmarks() -> str:
    ...
    # "Home Directory" first (replaces the hidden built-in "main" Home shortcut, item 4).
    lines = [f"{HOME_BOOKMARK_URI} {HOME_BOOKMARK_LABEL}"]
