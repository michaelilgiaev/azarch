...
# in lock-step with modifications/thunar/live_sidebar.SYNC_SCRIPT_DEST (a test pins them equal);
...
# single scale in modifications/scale). Every X client reads Xft.dpi as the screen DPI, so this
...
# The single scale source is modifications/scale; here it lands in the standard app-agnostic
...
def xresources() -> str:
    ...
    (modifications/scale). This is the fractional-scale backbone on X11 (GDK_SCALE is
    integer-only). `azarch display scale` rewrites the two values and re-runs `xrdb -merge`."""
    from modifications import scale
    return (
        "! Az'arch X resources. Generated by modifications/openbox (edit the Python, not this\n"
...
# EXPLICITLY from the single scale source (modifications.scale). pt(OPENBOX_TITLE_FONT_STOCK) ==
...
# gtk-xft-dpi -- see modifications/scale). GDK_SCALE stays 1 (integer-only; 1.35 is fractional).
...
PLAN = [
    ...
    },
    {
        ...
        # modifications/scale). Loaded by `xrdb -merge` in ~/.xinitrc. Home file, skel-mirrored.
        "builder": xresources,
