...
def kitty_conf() -> str:
    """Return the partial ~/.config/kitty/kitty.conf that sets the terminal font size and the
    ALT+<number> tab-switch keybinds.

    font_size is merged over kitty's built-in defaults, so every other kitty option stays
    stock (the same partial-config approach modifications/vlc.py uses for vlcrc). kitty.conf
    syntax is `<name> <value>` (space-separated, no '='). The size is kept equal to gedit's
    editor font size so the terminal and the editor match. The `map alt+N goto_tab N` lines
    make ALT+1..ALT+9 select tabs 1..9 and ALT+0 select tab 10 (see _alt_number_tab_binds)."""
    return f"""\
