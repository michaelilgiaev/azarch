"""Kitty terminal patch -- clean "> _" icon + an 18pt font_size (matching gedit).
...
way patches/vlc.py ships a partial vlcrc. It is a HOME file (owner "home"), skel-mirrored
...
# default), exactly the partial-config approach patches/vlc.py uses for vlcrc. The value is
...
KITTY_FONT_SIZE = 18   # pt; kept equal to gedit's editor font size (patches.gedit.GEDIT_FONT_SIZE)
...
def kitty_conf() -> str:
    ...
    kitty option stays stock (the same partial-config approach patches/vlc.py uses for
    ...
    return f"""\
# Az'arch kitty overrides. Generated by patches/kitty.py (edit the Python, not this file).
