...
# The set of recognized build variants -> iso_name. Threaded from compile.sh's
# --sshd flag through build.py/steps.py; see steps.run(variant=...).
ISO_NAMES = {
    "base": ISO_NAME,
    "sshd": ISO_NAME_SSHD,
}
...
FILE_PERMISSIONS = {
    ...
    # disabled"). This entry keeps the exec bit so the preview renders key legends.
