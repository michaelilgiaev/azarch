...
#
# To DOUBLE the bar we ship our own theme, "Azarch": a byte-for-byte copy of the stock
# Clearlooks openbox-3 themerc with only the size-driving fields grown. It is a fresh
# theme dir (not an edit of the packaged Clearlooks) so the airootfs overlay owns it and
# a package update to openbox cannot revert it:
#   * padding.height 2 -> 12   (top+bottom padding around the label -> taller bar)
#   * padding.width  3 -> 8     (matching horizontal breathing room)
#   * window.handle.width 3->6  (the bottom resize handle, kept proportional)
# The other half of the height comes from the larger title FONT set in rc.xml's <theme>
# (size 8 -> 16). Bigger font => taller label => OpenBox draws bigger buttons, so the
# min/max/close targets grow with the bar. Everything else (the #8CB0DC cyan gradient,
# the button gradients/hover/pressed states, the menu/osd styling) is copied verbatim so
# the look is identical, only larger.
...
# width. Pinned as constants so a test can prove the bar was actually doubled.
OPENBOX_THEME_PADDING_HEIGHT = 12   # stock Clearlooks: 2
OPENBOX_THEME_PADDING_WIDTH = 8     # stock Clearlooks: 3
OPENBOX_THEME_HANDLE_WIDTH = 6      # stock Clearlooks: 3
...
def openbox_theme_rc() -> str:
    ...
    the Clearlooks original so the bar keeps its familiar cyan look, just twice the
    height. Paired with the larger title <font> in rc.xml (size 16), this doubles the
    bar and the min/max/close buttons OpenBox sizes to the label.
    ...
    return f"""\
# Az'arch OpenBox theme -- Clearlooks with a DOUBLED titlebar. Generated by
...
# general stuff -- padding.height/width GROWN to double the titlebar (was 2 / 3),
...
# The title font size (points). The other half of the doubled titlebar: stock OpenBox
# defaults to 8pt for the window label; a 16pt label makes a taller bar AND (because
# OpenBox sizes the min/max/close buttons to the label height) bigger buttons. Set in
# rc.xml's <theme> <font> blocks below. Pinned so a test can prove it doubled.
TITLE_FONT_SIZE = 16
...
def openbox_rc_xml() -> str:
    ...
    Uses the Az'arch theme (Clearlooks with a DOUBLED titlebar, see openbox_theme_rc)
...
<openbox_config xmlns="http://openbox.org/3.4/rc">
  ...
  <theme>
    <!-- The Az'arch theme: Clearlooks with a doubled-height titlebar (openbox_theme_rc,
    ...
    <!-- Larger title font (the other half of the doubled bar): a taller label makes a
