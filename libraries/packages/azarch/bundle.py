...
patches.openbox.azarch_cli() calls bundle_source() and then re-injects the country table
between the AZARCH_CC markers from the single source of truth (patches/calamares/locale).
"""
...
def bundle_source() -> str:
    ...
    on-disk country table; patches.openbox.azarch_cli re-injects the canonical one)."""
    parts: list[str] = []
