...
Az'arch is [Arch Linux](https://archlinux.org) -- rolling release, `x86_64` -- with a curated package set and Az'arch branding/configuration on top. Every package comes unmodified from the official Arch repositories; Arch's own documentation, the [ArchWiki](https://wiki.archlinux.org), applies directly. The kernel is `linux` 7.1.5.arch1-2 and the init system is `systemd` 261.2-1. The medium is versioned date-based, YYYY.MM.DD (no semver).
...
Every external host and service the distribution talks to: where it downloads packages from, how it resolves the timezone and locale, and what it pings. Read live from the build configuration (`libraries/azarch/configuration/*.py`), so this list cannot drift from what the ISO actually does. The system is designed to work fully offline; these are the endpoints used **when a network is available**.
...
- **Timezone / locale / keyboard** are auto-detected on first boot from a geo-IP lookup; if that host is unreachable the defaults (`en_US.UTF-8`, `us`) apply. Change the provider in `libraries/azarch/configuration/locale.py`.
