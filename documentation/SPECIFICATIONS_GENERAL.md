...
General and developer-facing spec of the Az'arch distribution. The full base->top dependency graph (kernel up to leaf apps, tiered), with every component, lives in the companion image [`SPECIFICATIONS_COMPONENTS.svg`](SPECIFICATIONS_COMPONENTS.svg); this document is the prose and tables a developer of the distro actually needs. Generated by `scripts/pull_specifications.sh` from the official Arch `core`/`extra`/`multilib` databases.
