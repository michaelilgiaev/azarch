...
def test_plugin_metadata_matches_the_source_plugin_file():
    # The .plugin INI is read verbatim from the source tree (single source of truth) and
    # installed into gedit's plugin dir; its Module id must equal the active-plugins id.
...
def test_plugin_source_tree_is_present_and_buildable():
    # build_plugin() compiles these; the C source + Makefile + .plugin must exist so the ISO
    # build can produce the .so. (The actual compile happens on the build host in
    # compiler._emit_apps; here we just assert the inputs exist and the wiring points at
    # gedit's plugin dir.)
    srcs = {p.name for p in gedit._plugin_src_files()}
    assert "azarch-notepad.c" in srcs
    assert "Makefile" in srcs
    assert (gedit.GEDIT_PLUGIN_SRC_DIR / gedit.GEDIT_PLUGIN_METADATA_NAME).is_file()
