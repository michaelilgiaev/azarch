...
def test_emitter_family_covers_all_config_modules():
    ...
    # encrypted root is not prompted for twice at boot) + 21 desktop builders
    # (emit_plan: the 20 PLAN entries -- xinitrc, appletsrc, the org.kde.plasma.icon
    # menu backing .desktop (paper-icon fix), the application-menu daemon autostart
    # (instant first open), ksplashrc, plasmashellrc, kdeglobals, krunnerrc, kxkbrc,
    ...
    assert len(_EMITTERS) == 19 + 21 + 6 + 1 + 1 + 4 + 5
